...
```/create_account``` POST request to create a unique token for the client to save as an authentication key. The token is encrypted and added to a table with a numeric account id autogenerated

```/get_account_id```  POST request for autogenerated account id associated with token.

```/upload``` POST request to upload game data to server. Player stats are auto updated based on result.
...
```private/total-earnings-all``` POST request for total earnings for all players for a client.

```private/total-earnings-game``` POST request for total earnings for a specific game for a client.

```private/total-earnings-all``` POST request for total earnings for a specific player for a client.

```private/total-wins-all``` POST request for total wins for all players for a client.

```private/total-wins-game``` POST request for total wins for a specific game for a client.

```private/total-wins-player``` POST request for total wins for a specific player for a client.

```private/total-losses-all/<sessionId:string>/<host:string>``` Request total losses for all players for a client.

```private/total-losses-game```POST request for total losses for a specific game for a client.

```private/total-losses-player``` POST request for total losses for a specific player for a client.

```private/total-players-for-game``` POST request for total players for a specific game the client created.

```private/number-of-games``` POST request for total number of games for a specific game the client created.

```private/number-of-players``` POST request for total players for across all games the client created.

```private/greatest-player-by-wins``` POST request for total players for a specific game the client created.

```private/most-common-play``` POST request for most common result for a specific game created by the client.

```private/most-winning-play``` POST request for most common winning result for a specific game created by the client.
