...
For testing, the example client can be built with `make client` and run using ./client. The client requests a token from the server and stores it in a file for persistence when the client is used later on. The client has a command line interface which makes it possible to send commands and upload data or receive the requested statistics from the server. The command line interface exposes the entire API except for the /create_account and /get_account_id routes, which are not needed once a token is available.
